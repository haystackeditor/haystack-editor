...
(function (_nls) {
    ...
    function analyze(ts, contents, functionName, options = {}) {
        ...
        // all imports
        const imports = (0, lazy_js_1.default)(collect(ts, sourceFile, (n) => isImportNode(ts, n)
            ? CollectStepResult.YesAndRecurse
            : CollectStepResult.NoAndRecurse));
        ...
        const nlsLocalizeCallExpressions = importDeclarations
            ...
            .map((n) => n)
            // only `localize` calls
            .filter((n) => n.expression.kind === ts.SyntaxKind.PropertyAccessExpression &&
            n.expression.name.getText() ===
                functionName);
    ...
    class TextModel {
        ...
        toString() {
            return (0, lazy_js_1.default)(this.lines).zip(this.lineEndings).flatten().toArray().join("");
        }
    }
    ...
    function patch(ts, moduleId, typescript, javascript, sourcemap) {
        ...
        const nlsKeys = template(localizeCalls
            .map((lc) => lc.key)
            .concat(localize2Calls.map((lc) => lc.key)));
        const nls = template(localizeCalls
            .map((lc) => lc.value)
