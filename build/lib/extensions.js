...
function fromLocalWebpack(extensionPath, webpackConfigFileName, disableMangle) {
    ...
    vsce
        ...
        const webpackConfigLocations = glob.sync(path.join(extensionPath, "**", webpackConfigFileName), {
            ignore: ["**/node_modules"],
        });
...
function fromLocalNormal(extensionPath) {
    ...
    const result = es.through();
    vsce
        .listFiles({ cwd: extensionPath, packageManager: vsce.PackageManager.Yarn })
        .then((fileNames) => {
        const files = fileNames
            .map((fileName) => path.join(extensionPath, fileName))
            .map((filePath) => new vinyl_1.default({
