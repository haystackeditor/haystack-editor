...
export class CursorState {
	...
	public static fromModelSelection(modelSelection: ISelection, context: CursorContext): PartialModelCursorState {
		const selection = Selection.liftSelection(modelSelection);

		const selectionStart = selection.getSelectionStart()
		const selectionEnd = selection.getPosition()

		const modelState = new SingleCursorState(
			Range.fromPositions(selectionStart),
	...
	public static fromModelSelections(modelSelections: readonly ISelection[], context: CursorContext): PartialModelCursorState[] {
		const states: PartialModelCursorState[] = [];
		for (let i = 0, len = modelSelections.length; i < len; i++) {
			states[i] = this.fromModelSelection(modelSelections[i], context);
		}
		return states;
	}
