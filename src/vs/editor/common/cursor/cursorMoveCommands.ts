...
export class CursorMoveCommands {
	...
	public static selectAll(viewModel: IViewModel, cursor: CursorState): PartialCursorState {
		const lineCount = viewModel.model.getLineCount();
		const adjustedLine = viewModel.getEditRange() ? viewModel.getEditRange()!.endLineNumber : lineCount
		const maxColumn = viewModel.model.getLineMaxColumn(adjustedLine);

		return CursorState.fromModelState(new SingleCursorState(
			new Range(1, 1, 1, 1), SelectionStartKind.Simple, 0,
			new Position(adjustedLine, maxColumn), 0
		));
	}

	public static line(viewModel: IViewModel, cursor: CursorState, inSelectionMode: boolean, _position: IPosition, _viewPosition: IPosition | undefined, editRange: IRange | undefined): PartialCursorState {
		const tempPosition = new Position(_position.lineNumber, _position.column)
		const adjustedPosition = editRange ? tempPosition.delta(editRange.startLineNumber - 1) : tempPosition
		const validatedPosition = viewModel.model.validatePosition(adjustedPosition);
		const viewPosition = (
			_viewPosition
				? viewModel.coordinatesConverter.validateViewPosition(new Position(_viewPosition.lineNumber, _viewPosition.column), validatedPosition)
				: viewModel.coordinatesConverter.convertModelPositionToViewPosition(validatedPosition)
		);
		...
		if (!inSelectionMode) {
			...
			let selectToLineNumber = validatedPosition.lineNumber + 1;
			...
			return CursorState.fromModelState(new SingleCursorState(
				new Range(validatedPosition.lineNumber, 1, selectToLineNumber, selectToColumn), SelectionStartKind.Line, 0,
		...
		if (validatedPosition.lineNumber < enteringLineNumber) {

			return CursorState.fromViewState(cursor.viewState.move(
				true, viewPosition.lineNumber, 1, 0
			));

		} else if (validatedPosition.lineNumber > enteringLineNumber) {
	...
	public static word(viewModel: IViewModel, cursor: CursorState, inSelectionMode: boolean, _position: IPosition, editRange: IRange | undefined): PartialCursorState {
		const tempPosition = new Position(_position.lineNumber, _position.column)
		const adjustedPosition = editRange ? tempPosition.delta(editRange.startLineNumber - 1) : tempPosition
		const validatedPosition = viewModel.model.validatePosition(adjustedPosition);
		return CursorState.fromModelState(WordOperations.word(viewModel.cursorConfig, viewModel.model, cursor.modelState, inSelectionMode, validatedPosition));
	}
	...
	public static moveTo(viewModel: IViewModel, cursor: CursorState, inSelectionMode: boolean, _position: IPosition, _viewPosition: IPosition | undefined, editRange: IRange | undefined): PartialCursorState {
		if (inSelectionMode) {
			if (cursor.modelState.selectionStartKind === SelectionStartKind.Word) {
				return this.word(viewModel, cursor, inSelectionMode, _position, editRange);
			}
			if (cursor.modelState.selectionStartKind === SelectionStartKind.Line) {
				return this.line(viewModel, cursor, inSelectionMode, _position, _viewPosition, editRange);
			}
		}
