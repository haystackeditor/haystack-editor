...
export class LanguageFeatureRegistry<T> {
	...
	orderedWithoutModel(uri: URI, languageId: string): T[] {
		const result: T[] = [];
		this._orderedForEachWithoutModel(uri, languageId, entry => result.push(entry.provider));
		return result;
	}
	...
	private _orderedForEachWithoutModel(uri: URI, languageId: string, callback: (provider: Entry<T>) => any): void {
		this._updateScoresWithoutModel(uri, languageId);
	...
	private _updateScoresWithoutModel(uri: URI, languageId: string): void {
		...
		for (const entry of this._entries) {
			entry._score = score(entry.selector, candidate.uri, candidate.languageId, /* candidateIsSynchronized */ true, candidate.notebookUri, candidate.notebookType);
