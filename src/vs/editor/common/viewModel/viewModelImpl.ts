...
export class ViewModel extends Disposable implements IViewModel {
  ...
  private _viewportStart: ViewportStart
  private _lines: IViewModelLines
  public coordinatesConverter: ICoordinatesConverter
  ...
  constructor(
    ...
    model: ITextModel,
    private _editRange: IRange | undefined,
    private readonly _lineNumberOffset: number | undefined,
    ...
    if (
      ...
      this._lines = new ViewModelLinesFromModelAsIs(this.model, this._editRange)
    } else {
          ...
          for (const change of changes) {
            switch (change.changeType) {
              ...
              case textModelEvents.RawContentChangedType.LineChanged: {
                let injectedText: textModelEvents.LineInjectedText[] | null =
                  null
          ...
          for (const change of changes) {
            switch (change.changeType) {
              ...
              case textModelEvents.RawContentChangedType.LinesDeleted: {
                ...
                const changedToLineNumber =
                  change.toLineNumber -
                  (this._editRange ? this._editRange?.startLineNumber - 1 : 0)

                let linesDeletedEvent: viewEvents.ViewLinesDeletedEvent | null =
                  null
                const changedRange: IRange = {
                  startLineNumber: change.fromLineNumber,
                  startColumn: 1,
                  endLineNumber: change.toLineNumber,
                  endColumn: 1,
                }

                if (
                  this._editRange != null &&
                  Range.areIntersecting(this._editRange, changedRange)
                ) {
                  linesDeletedEvent = this._lines.onModelLinesDeleted(
                    versionId,
                    Math.max(changedFromLineNumber, 0),
                    Math.min(
                      changedToLineNumber,
                ...
                if (this._editRange) {
                  let beforeDelta = 0
                  let intersectDelta = 0

                  // For some reason, deleting a line is actually a composite action of changing a line and deleting a line.
                  // Therefore, we actually check if the changed line is in the edit range.
                  const adjustedChangedFromLineNumber =
                    changes.length > 1 &&
                    changes[0].changeType ===
                      textModelEvents.RawContentChangedType.LineChanged
                      ? changes[0].lineNumber
                      : change.fromLineNumber - 1
                  const adjustedChangedToLineNumber = change.toLineNumber - 1
                  ...
                  if (adjustedMinLineNumber < this._editRange.startLineNumber) {
                    beforeDelta =
                      Math.min(
                        this._editRange.startLineNumber,
                  ...
                  if (
                    minLineNumber >= this._editRange.startLineNumber &&
                    maxLineNumber <= this._editRange.endLineNumber
                  ) {
                    intersectDelta =
                      Math.min(maxLineNumber, this._editRange.endLineNumber) -
                      Math.max(minLineNumber, this._editRange.startLineNumber) +
                      1
                  }

                  this._editRange.startLineNumber -= beforeDelta
                  this._editRange.endLineNumber -= beforeDelta + intersectDelta

                  editRangeChanged = beforeDelta !== 0 || intersectDelta !== 0
                }
              ...
              case textModelEvents.RawContentChangedType.LinesInserted: {
                ...
                const changedFromLineNumber =
                  change.fromLineNumber -
                  (this._editRange ? this._editRange?.startLineNumber : 0)
                const changedToLineNumber =
                  change.toLineNumber -
                  (this._editRange ? this._editRange?.startLineNumber : 0)

                let linesInsertedEvent: viewEvents.ViewLinesInsertedEvent | null =
                  null

                if (
                  this._editRange != null &&
                  change.fromLineNumber > this._editRange.startLineNumber &&
                  change.fromLineNumber <= this._editRange.endLineNumber + 1
                ) {
                ...
                if (this._editRange) {
                  ...
                  if (minLineNumber <= this._editRange.startLineNumber) {
                    beforeDelta = maxLineNumber - minLineNumber + 1
                  }

                  // We added lines in the edit range, expand the range.
                  if (
                    minLineNumber > this._editRange.startLineNumber &&
                    minLineNumber <= this._editRange.endLineNumber + 1
                  ) {
                    // We don't constrain the line numbers here since
                    // we're adding a contiguous chunk inside the edit
                    // range.
                    intersectDelta = maxLineNumber - minLineNumber + 1
                  }
    ...
    this._register(
      this.model.onDidChangeTokens((e) => {
        ...
        for (let j = 0, lenJ = e.ranges.length; j < lenJ; j++) {
          ...
          if (
            this._editRange &&
            (modelRange.fromLineNumber < this._editRange.startLineNumber ||
              modelRange.fromLineNumber >= this._editRange?.endLineNumber ||
              modelRange.toLineNumber < this._editRange.startLineNumber ||
              modelRange.toLineNumber >= this._editRange?.endLineNumber)
          ) {
            continue
          }
          const fromLineNumber =
            modelRange.fromLineNumber -
            (this._editRange ? this._editRange.startLineNumber - 1 : 0)
          const toLineNumber =
  ...
  private _toModelVisibleRanges(visibleViewRange: Range): Range[] {
    ...
    if (hiddenAreas.length === 0) {
      const adjustedRange = this._editRange
        ? visibleRange.delta(this._editRange.startLineNumber - 1)
        : visibleRange
    ...
    const adjustedResults: Range[] = []

    for (const r of result) {
      const adjustedRange = this._editRange
        ? r.delta(this._editRange.startLineNumber - 1)
        : r
      adjustedResults.push(adjustedRange)
    }
  ...
  public getAllOverviewRulerDecorations(
    ...
    for (const decoration of decorations) {
      ...
      if (
        this._editRange &&
        decoration.range.intersectRanges(this._editRange) == null
      ) {
        continue
      }

      const decorationStartLineNumber = this._editRange
        ? Math.max(
  ...
  public getEditRange(): IRange | undefined {
    return this._editRange
  }
  public getLineNumberOffset(): number | undefined {
  ...
  public setSelections(
    ...
    if (this._editRange) {
      ...
      for (const selection of selections) {
        adjustedSelections.push({
          selectionStartLineNumber: Math.min(
            this._editRange.endLineNumber,
            Math.max(
  ...
  public setEditRange(editRange: IRange | null): void {
    this._editRange = editRange ?? undefined
...
class ViewportStart implements IDisposable {
  public static create(model: ITextModel, editRange?: IRange): ViewportStart {
