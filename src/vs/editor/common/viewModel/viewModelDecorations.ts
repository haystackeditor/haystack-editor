...
export class ViewModelDecorations implements IDisposable {
  ...
  private _getOrCreateViewModelDecoration(
    ...
    if (!r) {
      ...
      if (
        this._editRange &&
        modelRange.intersectRanges(this._editRange) == null
      ) {
        return new ViewModelDecoration(
          new Range(0, 0, 0, 0),
          modelDecoration.options,
        )
      }
      const adjustedRange = modelRange.delta(
      ...
      if (options.isWholeLine) {
        const start =
          this._coordinatesConverter.convertModelPositionToViewPosition(
            new Position(adjustedRange.startLineNumber, 1),
        ...
        const end =
          this._coordinatesConverter.convertModelPositionToViewPosition(
            new Position(
              adjustedRange.endLineNumber,
              this.model.getLineMaxColumn(adjustedRange.endLineNumber),
            ),
