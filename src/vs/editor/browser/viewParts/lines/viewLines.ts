  ...
  implements IVisibleLinesHost<ViewLine>, IViewLines
{
  ...
  public override onRevealRangeRequest(
    ...
    if (e.source === "api" && this._editRange && e.range != null) {
      e.range.startLineNumber += this._editRange.startLineNumber - 1
      e.range.endLineNumber += this._editRange.startLineNumber - 1
    }
  ...
  private _computeScrollTopToRevealRange(
    ...
    if (selections && selections.length > 0) {
      ...
      // For example, the vim extension causes degenrate behavior unless we add this check.
      if (
        source === "api" &&
        this._editRange &&
        range != null &&
    ...
    if (!shouldIgnoreScrollOff) {
      ...
      if (this._stickyScrollEnabled) {
        paddingTop =
          Math.max(context, this._maxNumberStickyLines) * this._lineHeight
      } else {
