...
export class ViewZones extends ViewPart {
	...
	private _computeWhitespaceProps(zone: IViewZone): IComputedViewZoneProps {
		...
		const adjustedZoneAfterModelPosition = editRange ? zoneAfterModelPosition.delta(-editRange.startLineNumber + 1) : zoneAfterModelPosition

		const viewPosition = this._context.viewModel.coordinatesConverter.convertModelPositionToViewPosition(adjustedZoneAfterModelPosition, zone.afterColumnAffinity, true);
