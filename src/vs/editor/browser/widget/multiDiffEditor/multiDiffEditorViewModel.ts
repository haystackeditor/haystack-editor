...
export class MultiDiffEditorViewModel extends Disposable {
  private readonly _documents = observableFromValueWithChangeEvent(
    this.model,
    this.model.documents,
  )

  public readonly items = mapObservableArrayCached(
    this,
    this._documents,
    (d, store) =>
      store.add(
  ...
  >(this, (reader, lastValue) => this.focusedDiffItem.read(reader) ?? lastValue)
