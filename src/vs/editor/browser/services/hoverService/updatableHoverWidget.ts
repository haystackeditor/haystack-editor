...
type IUpdatableHoverResolvedContent =
  | IMarkdownString
  | string
  | HTMLElement
  | undefined
...
export class UpdatableHoverWidget implements IDisposable {
  ...
  async update(
    content: IUpdatableHoverContent,
    focus?: boolean,
  ...
  private show(
    content: IUpdatableHoverResolvedContent,
    focus?: boolean,
    options?: IUpdatableHoverOptions,
  ): void {
  ...
  private hasContent(
    content: IUpdatableHoverResolvedContent,
  ): content is NonNullable<IUpdatableHoverResolvedContent> {
    if (!content) {
      return false
    }
