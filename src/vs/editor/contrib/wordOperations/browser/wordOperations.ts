...
export abstract class MoveWordCommand extends EditorCommand {
	...
	public runEditorCommand(accessor: ServicesAccessor, editor: ICodeEditor, args: any): void {
		...
		editor._getViewModel().setCursorStates('moveWordCommand', CursorChangeReason.Explicit, result.map(r => CursorState.fromModelSelection(r, new CursorContext(editor.getModel(), editor._getViewModel().getEditRange(), editor._getViewModel(), editor._getViewModel().coordinatesConverter, editor._getViewModel().cursorConfig))));
		if (result.length === 1) {
