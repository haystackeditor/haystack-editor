    ...
    if (arrayVisibleRanges.length !== 0) {
      ...
      for (const range of candidateRanges) {
        ...
        if (end - start > 0) {
          ...
          if (editRange != null) {
            if (
              start < editRange.startLineNumber ||
              start > editRange.endLineNumber
            ) {
              continue
            }
          }

          // We purposefully add 2 to the start since we don't want to block
          // the user from editing the first line of the symbol (since otherwise
          // the first line of the symbol would always be included).
          const adjustedTop = editRange
            ? start - editRange.startLineNumber + 2
            : start
          const adjustedBottom = editRange
            ? end - editRange.startLineNumber + 1
            : end
          const bottomOfBeginningLine =
            this._editor.getBottomForLineNumber(adjustedTop) - scrollTop
          const topOfEndLine = this._editor.getTopForLineNumber(end) - scrollTop
          const bottomOfEndLine =
            this._editor.getBottomForLineNumber(adjustedBottom) - scrollTop
