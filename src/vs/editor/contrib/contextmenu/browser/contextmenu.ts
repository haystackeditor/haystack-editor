  ...
  private _onContextMenu(e: IEditorMouseEvent): void {
    ...
    if (e.target.position) {
      ...
      if (!hasSelectionAtPosition) {
        ...
        const position = editRange
          ? e.target.position.delta(editRange.startLineNumber - 1)
          : e.target.position
    ...
    // Show the context menu. We wait for Haystack to potentially hijack the pointer though.
    const contextMenuBarrier = ContextMenuBarrier.getInstance()
    contextMenuBarrier.reset()
    contextMenuBarrier.getPromise().then((shouldContinue) => {
