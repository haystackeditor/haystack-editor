    ...
    const p = createCancelablePromise(async (token) => {
      ...
      try {
        ...
        if (
          ...
          providerEdits[0].provider instanceof
        ...
        if (providerEdits.length) {
          ...
          return this._postPasteWidgetManager.applyEditAndShowIfNeeded(
            ...
            canShowWidget,
            async (edit, token) => {
              const resolved = await edit.provider.resolveDocumentPasteEdit?.(
                edit,
                token,
              )
              if (resolved) {
      ...
      try {
        ...
        if (preference) {
          ...
          const selected = await this._quickInputService.pick(
            providerEdits.map(
              (edit): IQuickPickItem & { edit: DocumentPasteEdit } => ({
                label: edit.title,
  ...
  private async mergeInDataFromCopy(
    ...
    if (
      ...
      const toMergeDataTransfer =
        await CopyPasteController._currentCopyOperation.dataTransferPromise
