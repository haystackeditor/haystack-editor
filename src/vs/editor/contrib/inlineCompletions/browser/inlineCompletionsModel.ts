...
} from "vs/editor/contrib/inlineCompletions/browser/singleTextEdit"
  ...
  private async _acceptNext(
    ...
    try {
      ...
      try {
        ...
        const newText =
          editor.getModel()!.getValueInRange(replaceRange) + partialGhostTextVal
...
function getEndPositionsAfterApplying(
  ...
  const sortPerm = Permutation.createSortPermutation(edits, (edit1, edit2) =>
    Range.compareRangesUsingStarts(edit1.range, edit2.range),
  )
