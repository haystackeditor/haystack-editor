...
export class InlineCompletionsController extends Disposable {
  ...
  public readonly model = this._register(
    disposableObservableValue<InlineCompletionsModel | undefined>(
  ...
  private readonly _textModelVersionId = observableValue<
    ...
    VersionIdChangeReason
  >(this, -1)
  private readonly _positions = observableValueOpts<readonly Position[]>(
    { owner: this, equalsFn: itemsEquals(itemEquals()) },
    [new Position(1, 1)],
  )
          ...
          if (textModel) {
            const model = _instantiationService.createInstance(
              ...
              observableFromEvent(
                ...
                () => editor.getOption(EditorOption.suggest).preview,
              ),
              observableFromEvent(
                editor.onDidChangeConfiguration,
                () => editor.getOption(EditorOption.suggest).previewMode,
              ),
              observableFromEvent(
                editor.onDidChangeConfiguration,
                () => editor.getOption(EditorOption.inlineSuggest).mode,
              ),
              this._enabled,
            )
...
.monaco-editor .ghost-text {
  ...
  private updateObservables(
    ...
    this._positions.set(
      this.editor
        .getSelections()
        ?.map((selection) => selection.getPosition()) ?? [new Position(1, 1)],
      tx,
    )
  }
