...
} from "vs/editor/contrib/hover/browser/hoverActionIds"
...
namespace HoverAccessibilityHelpNLS {
  ...
  export const hoverContent = localize(
    "contentHover",
  ...
  implements IAccessibleViewContentProvider
{
  ...
  public onOpen(): void {
    ...
    this._markdownHoverFocusedIndex =
      this._hoverController.focusedMarkdownHoverIndex()
    this._onHoverContentsChanged = this._register(
  ...
  implements IAccessibleViewContentProvider
{
  ...
  private _descriptionOfVerbosityActionForIndex(
    ...
    const isActionSupported =
      this._hoverController.doesMarkdownHoverAtIndexSupportVerbosityAction(
        index,
        action,
      )
  ...
  protected _descriptionOfFocusedMarkdownHoverAtIndex(index: number): string[] {
    const content: string[] = []
    const hoverContent =
      this._hoverController.markdownHoverContentAtIndex(index)
    if (hoverContent) {
  ...
  implements IAccessibleViewContentProvider
{
  ...
  public provideContent(): string {
    ...
    return hoverContent.length > 0
      ? hoverContent
      : this._hoverController.getWidgetContent() ||
  ...
  private _getActionFor(
    ...
    return new Action(
      ...
      () => {
        editor
          .getAction(actionId)
          ?.run({ index: this._markdownHoverFocusedIndex, focus: false })
      },
    )
  }
  ...
  private _initializeOptions(
    ...
    this.options.customHelp = () => {
      return helpProvider.provideContentAtIndex(this._markdownHoverFocusedIndex)
    }
  }
}
