...
export class LinkDetector extends Disposable implements IEditorContribution {
  ...
  public openLinkOccurrence(
    ...
    link.resolve(CancellationToken.None).then(
      (uri) => {
        ...
        const uriLink = URI.isUri(uri) ? uri : URI.parse(uri)
        ...
        if (uriLink.scheme === Schemas.file) {
          ...
          this.haystackService.createFileEditor(uri, {
  ...
  public getLinkOccurrence(position: Position | null): LinkOccurrence | null {
    ...
    const lineDelta = editRange ? editRange.startLineNumber - 1 : 0

    const decorations = this.editor.getModel().getDecorationsInRange(
      {
        startLineNumber: position.lineNumber + lineDelta,
        startColumn: position.column,
        endLineNumber: position.lineNumber + lineDelta,
