...
import {
  ...
  LineData,
} from "vs/editor/contrib/inlineCompletions/browser/ghostTextWidget"
...
export class GhostTextWidget extends Disposable {
  ...
  private readonly decorations = derived(this, (reader) => {
    ...
    if (uiState.range) {
      ...
      if (uiState.isSingleLine) {
        ...
        for (let i = 0; i < lines; i++) {
          ...
          const firstNonWhitespace =
            uiState.targetTextModel.getLineFirstNonWhitespaceColumn(line)
          const lastNonWhitespace =
            uiState.targetTextModel.getLineLastNonWhitespaceColumn(line)
      ...
      const className = uiState.backgroundColoring
        ? "inline-edit-remove backgroundColoring"
        : "inline-edit-remove"
