...
export class SuggestWidget implements IDisposable {
	...
	showTriggered(auto: boolean, delay: number) {
		...
		const editorPosition = editRange ? (this.editor.getPosition()?.delta(editRange.startLineNumber - 1) ?? null) : this.editor.getPosition()
		this._contentWidget.setPosition(editorPosition);
		this._isAuto = !!auto;
