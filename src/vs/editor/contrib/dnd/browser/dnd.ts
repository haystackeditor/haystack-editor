		...
		if (mouseEvent.target && (this._hitContent(mouseEvent.target) || this._hitMargin(mouseEvent.target)) && mouseEvent.target.position) {
			...
			if (this._dragSelection === null) {
				...
				if (mouseEvent.event.shiftKey) {
					...
					const editRange = this._editor._getViewModel()?.getEditRange()
					const adjustedNewPosition = editRange ? newCursorPosition.delta(editRange.startLineNumber - 1) : newCursorPosition
					newSelections = (this._editor.getSelections() || []).map(selection => {
						if (selection.containsPosition(adjustedNewPosition)) {
							return new Selection(adjustedNewPosition.lineNumber, adjustedNewPosition.column, adjustedNewPosition.lineNumber, adjustedNewPosition.column);
						} else {
							return selection;
						}
					});
