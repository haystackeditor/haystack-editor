...
import { renderHaystackApp } from "vs/workbench/browser/haystack-frontend/root/app"
...
export class EditorPart extends Part implements IEditorPart, IEditorGroupsView {
  ...
  private doCreateGridControl(): void {
    ...
    if (!this.gridWidget || restoreError || this.activeGroup == null) {
  ...
  private async doApplyState(
    ...
    const resumeEvents = this.disposeGroups(
      true /* suspress events for the duration of applying state */,
    )
