...
export class EditorGroupView extends Themable implements IEditorGroupView {
  ...
  private handleGroupContextKeys(): void {
    ...
    const multipleEditorsSelectedContext =
      MultipleEditorsSelectedInGroupContext.bindTo(this.scopedContextKeyService)
  ...
  private restoreEditors(
    ...
    if (!activeEditor || activeEditor.resource == null) {
      return
    }
  ...
  private doShowEditor(
    ...
    if (context.active) {
      openEditorPromise = (async () => {
        const result = await this.editorPane.openEditor(
          editor,
          options,
          internalOptions,
        ...
        const { pane, changed, cancelled, error } = result
  ...
  async openEditors(
    ...
    const editorResults = await Promises.settled(
    ...
    if (this.editorPane.activeEditorPane != null) {
      const foundActiveEditorPane = editorResults.find(
        (pane) => pane?.getId() === this.editorPane.activeEditorPane?.getId(),
      )
      return foundActiveEditorPane
    }
  ...
  async closeEditor(
    ...
    if (options?.closeUnconditionally) {
      // Revert first, then close.
  ...
  private doCloseEditor(
    ...
    this.editorPane.closeEditorIrrespectiveOfActive(editor)
  ...
  async replaceEditors(editors: EditorReplacement[]): Promise<void> {
    ...
    // We do not have the notion of inactive editors, so do not handle them.
    if (activeReplacement) {
      // Open replacement as active editor
      // Make sure we set active editor to null first.
      this.model.clearActiveEditor()

      await this._haystackService.createFileEditor(
        activeReplacement.replacement.resource,
        {
          replaceExistingEditorInput: {
            input: activeReplacement.editor,
            groupId: this.id,
          },
        },
  ...
  override updateStyles(): void {
    ...
    // Title control
    const borderColor =
      this.getColor(EDITOR_GROUP_HEADER_BORDER) || this.getColor(contrastBorder)
  ...
  shouldSerialize(): boolean {
    let hasOnlyGhostEditors = true
    for (const editor of this.editorPane.getEditorPanes()) {
      if (editor.input == null) continue
      if (!this._haystackService.getIsGhostEditor(editor.input)) {
        hasOnlyGhostEditors = false
        break
      }
    }
    return !this.isEmpty && !hasOnlyGhostEditors
  }

  toJSON(): ISerializedEditorGroupModel {
    for (const editor of this.model.getEditors(EditorsOrder.SEQUENTIAL)) {
      const isGhost = this._haystackService.getIsGhostEditor(editor)
      editor.setIsGhost(isGhost)

      const transformInfo =
        this._haystackService.getTransformInfoForEditor(editor)
      if (transformInfo == null) continue
      const { xPosition, yPosition, width, height } = transformInfo
      editor.setPosition(xPosition, yPosition)
