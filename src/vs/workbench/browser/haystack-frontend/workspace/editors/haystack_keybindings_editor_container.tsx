...
import { HaystackKeybindingsEditor } from "vs/workbench/browser/haystack-frontend/editor/haystack_keybindings_editor"
...
interface HaystackKeybindingsEditorContainerProps {
  editorId: string
}

// This is a dummy container used to prevent overly re-rendering due to camera/editor
// position or dimensions changes.
export function HaystackKeybindingsEditorContainer({
  editorId,
}: HaystackKeybindingsEditorContainerProps) {
  ...
  return (
    ...
    >
      <HaystackKeybindingsEditorContainerImpl editorId={editorId} />
    </div>
  )
}

export const HaystackKeybindingsEditorContainerImpl = React.memo(
  function CopilotSettingsEditorContainerImpl({
    ...
    const { isPinned } = WorkspaceStoreWrapper.useWorkspaceState(
      (state) => state.idToEditorMap.get(editorId) as CanvasCodeEditor,
    )
    ...
    return (
      ...
      >
        <EditorHeader
          fileName={"Haystack Keybindings"}
          relativeFilePath={"Haystack Keybindings"}
          icon={<SmallFileSvg />}
          ...
          editorId={editorId}
        />
        <HaystackKeybindingsEditor editorId={editorId} />
