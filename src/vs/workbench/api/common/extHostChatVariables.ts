...
export class ExtHostChatVariables implements ExtHostChatVariablesShape {
  ...
  registerVariableResolver(
    ...
    this._resolver.set(handle, {
      extension,
      data: { id, name, description: userDescription, modelDescription, icon },
      resolver: resolver,
    })
  ...
  attachContext(
    name: string,
    value: string | vscode.Location | vscode.Uri | unknown,
    location: vscode.ChatLocation.Panel,
  ) {
    this._proxy.$attachContext(
      name,
      extHostTypes.Location.isLocation(value)
        ? typeConvert.Location.from(value)
        : value,
      typeConvert.ChatLocation.from(location),
    )
  }
}
