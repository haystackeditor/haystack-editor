  ...
  implements ExtHostChatAgentsShape2
{
  ...
  private async prepareHistoryTurns(
    ...
    for (const h of context.history) {
      ...
      // REQUEST turn
      res.push(
        new extHostTypes.ChatRequestTurn(
          h.request.message,
          h.request.command,
          h.request.variables.variables.map(
            typeConvert.ChatAgentValueReference.to,
          ),
          h.request.agentId,
        ),
      )
      ...
      const parts = coalesce(
        h.response.map((r) =>
          typeConvert.ChatResponsePart.toContent(r, this.commands.converter),
        ),
      )
      res.push(
...
class ExtHostChatAgent {
  ...
  public isAgentEnabled() {
    ...
    return !(
      this.quality === "stable" &&
      !isProposedApiEnabled(this.extension, "chatParticipantPrivate")
    )
  }
