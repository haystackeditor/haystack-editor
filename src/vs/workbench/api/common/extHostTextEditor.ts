...
export class ExtHostTextEditor {
  ...
  constructor(
    ...
    if (editRange != null) {
      this._editRange = {
        ...
        endLineNumber: editRange.endLineNumber - 1,
        endColumn: editRange.endColumn,
      }
    }
    ...
    this.value = Object.freeze({
      ...
      get editRange(): IRange | undefined {
        return that._editRange
      },
      // --- selection
      ...
      set selection(value: Selection) {
        ...
        const adjustedSelections: Selection[] = []

        if (that._editRange != null) {
          for (const selection of [value]) {
            adjustedSelections.push(
              new Selection(
                new Position(
                  Math.min(
                    that._editRange.endLineNumber - 1,
                    Math.max(
                      selection.start.line,
                      that._editRange.startLineNumber - 1,
                    ),
                  ),
          ...
          adjustedSelections.push(value)
        }

        that._selections = adjustedSelections
  ...
  _acceptEditRange(value: IRange | undefined): void {
    ok(!this._disposed)
    this._editRange = value
  }
