...
class DocumentPasteEditProvider {
  ...
  async providePasteEdits(
    ...
    const dataTransfer = typeConvert.DataTransfer.toDataTransfer(
      ...
      async (id) => {
        return (
          await this._proxy.$resolvePasteFileData(this._handle, requestId, id)
        ).buffer
      },
    )
  ...
  implements extHostProtocol.ExtHostLanguageFeaturesShape
{
  ...
  $provideInlineEdit(
    ...
    return this._withAdapter(
      ...
      InlineEditAdapter,
      (adapter) =>
        adapter.provideInlineEdits(URI.revive(resource), context, token),
      undefined,
      token,
    )
  }
  ...
  $provideMappedEdits(
    ...
    return this._withAdapter(
      ...
      (adapter) =>
        Promise.resolve(
          adapter.provideMappedEdits(document, codeBlocks, context, token),
        ),
      null,
      token,
    )
