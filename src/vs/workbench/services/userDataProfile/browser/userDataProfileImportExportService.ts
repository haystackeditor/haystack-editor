  ...
  implements IUserDataProfileImportExportService, IURLHandler
{
  ...
  async importProfile(
    ...
    disposables.add(
      toDisposable(() => this.isProfileImportInProgressContextKey.set(false)),
    )
  ...
  private async saveProfile(
      ...
      quickPick.onDidChangeSelection((items) => {
        ...
        for (const resource of resources) {
          ...
          const description = resource.picked
            ? undefined
            : localize("use default profile", "Using Default Profile")
        ...
        const index = profileOptions.findIndex((option) => {
          if (source instanceof URI) {
            return (
              ...
              this.uriIdentityService.extUri.isEqual(option.source, source)
            )
    ...
    try {
      ...
      if (profile) {
        await this.userDataProfileManagementService.updateProfile(profile, {
          name: result.name,
          icon: result.icon,
          useDefaultFlags:
        ...
        if (source instanceof URI) {
          ...
          this.telemetryService.publicLog2<
            CreateProfileInfoEvent,
            CreateProfileInfoClassification
          >("userDataProfile.createFromProfile", createProfileTelemetryData)
          ...
          await this.createAndSwitch(
            ...
            false,
            true,
            { useDefaultFlags, icon: result.icon ? result.icon : undefined },
            localize("create profile", "Create Profile"),
          )
        } else {
          this.telemetryService.publicLog2<
            CreateProfileInfoEvent,
            CreateProfileInfoClassification
          >("userDataProfile.createEmptyProfile", createProfileTelemetryData)
      ...
      await this.progressService.withProgress(
        ...
        async (progress) => {
          ...
          const createdProfile = await this.doCreateProfile(
            ...
            false,
            false,
            { useDefaultFlags: options?.useDefaultFlags, icon: options?.icon },
            reportProgress,
          )
          if (createdProfile) {
  ...
  private async doExportProfile(
    ...
    const disposables = new DisposableStore()
    disposables.add(
      toDisposable(() => this.isProfileExportInProgressContextKey.set(false)),
    )
    ...
    try {
      ...
      const primaryAction = isWeb
        ? new Action(
            ...
            undefined,
            true,
            async () => this.openerService.open(uri, { openExternal: true }),
          )
        : importAction
      ...
      message.appendMarkdown(
        `[${localize("learn more", "Learn more")}](https://aka.ms/vscode-extension-marketplace#_can-i-trust-extensions-from-the-marketplace).`,
      )
                ...
                async (progress) => {
                  ...
                  if (profileTemplate.extensions) {
                    await that.instantiationService
                      .createInstance(ExtensionsResource)
                      .apply(profileTemplate.extensions, importedProfile)
                  }
                  disposable.dispose()
                },
              )
      ...
      disposables.add(
        Event.debounce(
          ...
          const profileTemplate =
            await userDataProfileImportState.getProfileTemplateToImport()
          if (profileTemplate.extensions) {
            const profileExtensions = await that.instantiationService
              .createInstance(ExtensionsResource)
              .getProfileExtensions(profileTemplate.extensions)
            const installed =
            ...
            if (
              profileExtensions.every((e) =>
                installed.some((i) =>
                  areSameExtensions(e.identifier, i.identifier),
                ),
              )
            ) {
              disposable.dispose()
    ...
    if (profile) {
      ...
      const name = await this.quickInputService.input({
        ...
        validateInput: async (value: string) => {
          if (
            this.userDataProfilesService.profiles.some((p) => p.name === value)
          ) {
            return localize(
              "profileExists",
              "Profile with name {0} already exists.",
  ...
  private async closeAllImportExportPreviewEditors(): Promise<void> {
    const editorsToColse = this.editorService
      ...
      .filter(
        ({ editor }) =>
          editor.resource?.scheme === USER_DATA_PROFILE_EXPORT_SCHEME ||
          editor.resource?.scheme === USER_DATA_PROFILE_EXPORT_PREVIEW_SCHEME ||
          editor.resource?.scheme === USER_DATA_PROFILE_IMPORT_PREVIEW_SCHEME,
      )
    ...
    await this.progressService.withProgress(
      ...
      async (progress) => {
        if (profile.settings) {
          await this.instantiationService
            .createInstance(SettingsResource)
            .apply(profile.settings, this.userDataProfileService.currentProfile)
        }
