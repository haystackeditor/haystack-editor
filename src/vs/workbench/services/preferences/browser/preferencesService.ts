  ...
  implements IPreferencesService
{
  ...
  async openRawDefaultSettings(): Promise<IEditorPane | undefined> {
    const input: IResourceEditorInput = {
      resource: this.defaultSettingsRawResource,
    }
    await this.haystackService.createModalEditor(input)
    return undefined
  }

  async openRawUserSettings(): Promise<IEditorPane | undefined> {
    await this.haystackService.createFileEditor(this.userSettingsResource)
    return undefined
  }
  ...
  async openGlobalKeybindingSettings(
    ...
    if (textual) {
      ...
      if (openDefaultKeybindings) {
        ...
        await Promise.all([
          this.haystackService.createModalEditor(input),
  ...
  async openDefaultKeybindingsFile(): Promise<IEditorPane | undefined> {
    const input: IResourceEditorInput = {
      resource: this.defaultKeybindingsResource,
    }
    await this.haystackService.createModalEditor(input)
