  ...
  implements ITextFileService
{
  ...
  private readonly _onDidSave = this._register(new Emitter<URI>())
  ...
  async save(
    ...
    if (resource.scheme === Schemas.untitled) {
      ...
      if (model) {
        ...
        if (targetUri) {
          const result = await this.saveAs(resource, targetUri, options)
          if (result != null) {
            this._onDidSave.fire(result)
          }
          return result
        }
      }
    ...
    // File
      ...
      if (model) {
        const result = (await model.save(options)) ? resource : undefined
        if (result != null) {
          this._onDidSave.fire(result)
        }
        return result
  ...
  dirtyModel(resource: URI): void {
    const model =
      resource.scheme === Schemas.untitled
        ? this.untitled.get(resource)
        : this.files.get(resource)
    if (model && isTextFileEditorModel(model)) {
      model.setDirty(true)
    }
  }
