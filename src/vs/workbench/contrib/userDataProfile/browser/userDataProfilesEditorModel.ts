...
export abstract class AbstractUserDataProfileElement extends Disposable {
  ...
  ): Promise<IProfileResourceChildTreeItem[]> {
    return []
  }
...
export class UserDataProfileElement
  extends AbstractUserDataProfileElement
  implements IProfileElement
{
  ...
  private async doSave(): Promise<void> {
    ...
    await this.userDataProfileManagementService.updateProfile(this.profile, {
      ...
      icon: this.icon,
      useDefaultFlags:
        this.profile.useDefaultFlags && !useDefaultFlags ? {} : useDefaultFlags,
    })
  }
...
export class NewProfileElement
  extends AbstractUserDataProfileElement
  implements IProfileElement
{
  constructor(
...
export class UserDataProfilesEditorModel extends EditorModel {
  ...
  private createProfileElement(
    ...
    const copyFromProfileAction = disposables.add(
      new Action(
        ...
        localize("copyFromProfile", "Save As..."),
  ...
  createNewProfile(copyFrom?: URI | IUserDataProfile): IProfileElement {
    if (!this.newProfileElement) {
      const disposables = new DisposableStore()
      const discardAction = disposables.add(
        new Action(
          "userDataProfile.discard",
  ...
  async saveNewProfile(): Promise<void> {
    ...
    if (copyFrom instanceof URI) {
      ...
      await this.userDataProfileImportExportService.importProfile(copyFrom, {
        mode: "apply",
        name: name,
        useDefaultFlags,
