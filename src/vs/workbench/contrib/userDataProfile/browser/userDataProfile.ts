...
} from "vs/workbench/contrib/userDataProfile/browser/userDataProfilesEditor"
  ...
  implements IWorkbenchContribution
{
  ...
  private registerManageProfilesAction(): IDisposable {
    ...
    const when = ContextKeyExpr.equals(
      "config.workbench.experimental.enableNewProfilesUI",
      true,
    )
    disposables.add(
      registerAction2(
      ...
      class ProfileEntryAction extends Action2 {
        ...
        async run(accessor: ServicesAccessor) {
          if (that.userDataProfileService.currentProfile.id !== profile.id) {
            if (
              profile.isDefault &&
              Date.now() - that.startTime < 1000 * 20 /* 20 seconds */
            ) {
              type SwitchToDefaultProfileInfoClassification = {
                ...
                emptyWindow: {
                  ...
                  comment: "If the current window is empty window or not"
                }
              }
              ...
              that.telemetryService.publicLog2<
                SwitchToDefaultProfileInfoEvent,
                SwitchToDefaultProfileInfoClassification
              >("profiles:newwindowprofile", {
                emptyWindow:
                  that.workspaceContextService.getWorkbenchState() ===
    ...
    return registerAction2(
      class SwitchProfileAction extends Action2 {
        ...
        async run(accessor: ServicesAccessor) {
          ...
          const actions =
            menu.getActions().find(([group]) => group === "0_profiles")?.[1] ??
            []
          try {
                ...
                try {
                  if ((<IProfileTemplateQuickPickItem>selectedItem).url) {
                    return await that.userDataProfileImportExportService.createProfile(
                      URI.parse(
                        (<IProfileTemplateQuickPickItem>selectedItem).url,
                      ),
                    )
                  }
                  const profile =
