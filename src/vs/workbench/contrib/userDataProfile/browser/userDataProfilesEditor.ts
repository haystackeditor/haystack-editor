...
} from "vs/workbench/contrib/userDataProfile/browser/userDataProfilesEditorModel"
  ...
  implements IUserDataProfilesEditor
{
  ...
  protected createEditor(parent: HTMLElement): void {
    ...
    this.userDataProfileManagementService
  ...
  private renderSidebar(parent: HTMLElement): void {
    ...
    const renderer = this.instantiationService.createInstance(
      ProfileTreeElementRenderer,
    )
    const delegate = new ProfileTreeElementDelegate()
    this.profilesTree = this._register(
      this.instantiationService.createInstance(
        WorkbenchObjectTree<IProfileElement>,
        "ProfilesTree",
  ...
  private registerListeners(): void {
    if (this.profilesTree) {
      this._register(
        this.profilesTree.onDidChangeSelection((e) => {
      ...
      this._register(
        this.profilesTree.onContextMenu((e) => {
          if (e.element instanceof AbstractUserDataProfileElement) {
            this.contextMenuService.showContextMenu({
              getAnchor: () => e.anchor,
              getActions: () =>
                e.element instanceof AbstractUserDataProfileElement
                  ? e.element.contextMenuActions.slice(0)
                  : [],
  ...
  private updateProfilesTree(elementToSelect?: IProfileElement): void {
    ...
    const profileElements: IObjectTreeElement<IProfileElement>[] =
      this.model.profiles.map((element) => ({ element }))
...
class ProfileTreeElementDelegate
  implements IListVirtualDelegate<IProfileElement>
{
  getHeight(element: IProfileElement) {
    return 30
  }
  getTemplateId() {
    return "profileTreeElement"
  }
}

class ProfileTreeElementRenderer
  implements
    ITreeRenderer<IProfileElement, void, IProfileTreeElementTemplateData>
{
  ...
  renderElement(
    { element }: ITreeNode<IProfileElement, void>,
    index: number,
    templateData: IProfileTreeElementTemplateData,
...
class ProfileWidget extends Disposable {
  ...
  private copyFromOptions: (ISelectOptionItem & {
    id?: string
    source?: IUserDataProfile | URI
  })[] = []

  private readonly resourcesTree: WorkbenchAsyncDataTree<
    AbstractUserDataProfileElement,
  ...
  constructor(
    ...
    this.toolbar = this._register(
      instantiationService.createInstance(WorkbenchToolBar, actionsContainer, {
    ...
    this.nameInput = this._register(
      new InputBox(this.nameContainer, undefined, {
        ...
        validationOptions: {
          validation: (value) => {
            ...
            const initialName =
              ...
              UserDataProfileElement
                ? this._profileElement.value.element.profile.name
                : undefined
            if (
              initialName !== value &&
    ...
    append(
      this.copyFromContainer,
      $(
        ".profile-copy-from-label",
        undefined,
        localize("create from", "Copy from:"),
      ),
    )
    this.copyFromSelectBox = this._register(
      this.instantiationService.createInstance(
        SelectBox,
        [],
        0,
    ...
    this.copyFromSelectBox.render(
      append(this.copyFromContainer, $(".profile-select-container")),
    )

    const contentsContainer = append(body, $(".profile-contents-container"))
    append(
      contentsContainer,
      $(".profile-contents-label", undefined, localize("contents", "Contents")),
    )

    const delegate = new ProfileResourceTreeElementDelegate()
    this.resourcesTree = this._register(
      this.instantiationService.createInstance(
        WorkbenchAsyncDataTree<
          AbstractUserDataProfileElement,
          ProfileResourceTreeElement
    ...
    this._register(
      this.resourcesTree.onDidOpen(async (e) => {
        if (!e.browserEvent) {
          return
        }
        if (
        ...
        if (e.element && !isString(e.element.element)) {
          if (e.element.element.resourceUri) {
            await this.commandService.executeCommand(
              ...
              e.element.element.resourceUri,
              [SIDE_GROUP],
              undefined,
              e,
            )
          } else if (
            e.element.element.parent instanceof ExtensionsResourceTreeItem
          ) {
            await this.commandService.executeCommand(
              "extension.open",
  ...
  private renderIconSelectBox(iconContainer: HTMLElement): void {
    const iconSelectBox = this._register(
    ...
    const showIconSelectBox = () => {
      if (
        this._profileElement.value?.element instanceof UserDataProfileElement &&
        this._profileElement.value.element.profile.isDefault
      ) {
        return
      }
    ...
    this._register(
      addDisposableListener(iconContainer, EventType.CLICK, (e: MouseEvent) => {
        EventHelper.stop(e, true)
        showIconSelectBox()
      }),
    )
    this._register(
      addDisposableListener(iconContainer, EventType.KEY_DOWN, (e) => {
  ...
  private renderSelectBox(): void {
    ...
    this._register(
      this.copyFromSelectBox.onDidSelect((option) => {
        if (this._profileElement.value?.element instanceof NewProfileElement) {
          this._profileElement.value.element.copyFrom =
            this.copyFromOptions[option.index].source
        }
      }),
    )
  }
  ...
  layout(dimension: Dimension): void {
    this.resourcesTree.layout(
      dimension.height - 34 - 20 - 25 - 20,
      dimension.width,
    )
  }
  ...
  render(profileElement: AbstractUserDataProfileElement): void {
    ...
    if (profileElement.primaryAction) {
      ...
      disposables.add(
        button.onDidClick(() =>
          this.editorProgressService.showWhile(
            profileElement.primaryAction!.run(),
          ),
        ),
      )
      disposables.add(
        profileElement.primaryAction.onDidChange((e) => {
      ...
      disposables.add(
        profileElement.onDidChange((e) => {
          if (e.message) {
            button.setTitle(
              profileElement.message ?? profileElement.primaryAction!.label,
            )
    ...
    this.toolbar.setActions(
      profileElement.titleActions[0].slice(0),
      profileElement.titleActions[1].slice(0),
    )
  ...
  private renderProfileElement(
    profileElement: AbstractUserDataProfileElement,
  ): void {
    this.profileTitle.textContent = profileElement.name
    this.nameInput.value = profileElement.name
    if (profileElement.icon) {
      this.iconElement.className = ThemeIcon.asClassName(
        ThemeIcon.fromId(profileElement.icon),
      )
    } else {
      this.iconElement.className = ThemeIcon.asClassName(
    ...
    if (profileElement instanceof NewProfileElement) {
      ...
      if (index !== -1) {
        ...
        this.copyFromSelectBox.setOptions([
          { text: basename(profileElement.copyFrom as URI) },
        ])
...
class ProfileResourceTreeElementDelegate
  implements IListVirtualDelegate<ProfileResourceTreeElement>
{
    ...
    >
{
  ...
  hasChildren(
    element: AbstractUserDataProfileElement | ProfileResourceTreeElement,
  ): boolean {
    if (element instanceof AbstractUserDataProfileElement) {
      return true
    }
  ...
  async getChildren(
    element: AbstractUserDataProfileElement | ProfileResourceTreeElement,
  ): Promise<ProfileResourceTreeElement[]> {
    if (element instanceof AbstractUserDataProfileElement) {
      const resourceTypes = [
...
class AbstractProfileResourceTreeRenderer extends Disposable {
  ...
  disposeElement(
    element: ITreeNode<ProfileResourceTreeElement, void>,
    index: number,
    templateData: IProfileResourceTemplateData,
    height: number | undefined,
  ): void {
    ...
    >
{
  ...
  renderTemplate(parent: HTMLElement): IExistingProfileResourceTemplateData {
    ...
    const description = append(
      ...
      $(
        ...
        undefined,
        localize("using defaults", "Using Default Profile"),
      ),
    )
    return {
      checkbox,
  ...
  renderElement(
    ...
    }: ITreeNode<ProfileResourceTreeElement, void>,
    index: number,
    ...
    if (root instanceof UserDataProfileElement && root.profile.isDefault) {
      ...
      templateData.elementDisposables.add(
        templateData.checkbox.onChange(() =>
          root.setFlag(element, !templateData.checkbox.checked),
        ),
      )
      templateData.elementDisposables.add(
        root.onDidChange((e) => {
    ...
    >
{
  ...
  renderTemplate(parent: HTMLElement): INewProfileResourceTemplateData {
    ...
    const selectBox = this._register(
      this.instantiationService.createInstance(
        SelectBox,
        [
  ...
  renderElement(
    ...
    if (!isString(element)) {
      throw new Error(
        "NewProfileResourceTreeRenderer can only profile resoyrce types",
      )
    }
    templateData.label.textContent = this.getResourceTypeTitle(element)
    templateData.selectBox.select(
      root.getCopyFlag(element) ? 1 : root.getFlag(element) ? 2 : 0,
    )
    templateData.elementDisposables.add(
      templateData.selectBox.onDidSelect((option) => {
    ...
    templateData.resourceLabel.setResource(
      {
        ...
        description: isString(element.description)
          ? element.description
          : undefined,
        resource,
      },
