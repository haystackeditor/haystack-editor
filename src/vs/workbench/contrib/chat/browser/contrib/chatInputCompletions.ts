          ...
          provideCompletionItems: async (
            ...
            if (
              ...
              (widget.location !== ChatAgentLocation.Panel &&
                widget.location !==
                  ChatAgentLocation.Notebook) /* TODO@jrieken - enable when agents are adopted*/
            ) {
              return null
            }
      ...
      this.languageFeaturesService.completionProvider.register(
        { scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true },
        {
          ...
          provideCompletionItems: async (
            ...
            if (
              usedAgent &&
              !Range.containsPosition(usedAgent.editorRange, position)
            ) {
            ...
            return {
              suggestions: agents.map((agent, i): CompletionItem => {
                const { label: agentLabel, isDupe } = getAgentCompletionDetails(
                  agent,
                  agents,
                  this.chatAgentNameService,
                )
                return {
        ...
        { scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true },
        {
          ...
          provideCompletionItems: async (
            ...
            if (
              ...
              !widget.viewModel ||
              widget.location !==
                ChatAgentLocation.Panel /* TODO@jrieken - enable when agents are adopted*/
            ) {
              return
            }
        ...
        { scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true },
        {
          ...
          provideCompletionItems: async (
            ...
            const getFilterText = (agent: IChatAgentData, command: string) => {
              ...
              const dummyPrefix =
                agent.id === "github.copilot.terminal" ? `0000` : ``
          ...
          provideCompletionItems: async (
            ...
            const variableItems = Array.from(
              ...
              .filter(
                (v) =>
                  !usedVariables.some(
                    (usedVar) => usedVar.variableName === v.name,
                  ),
              )
...
function getAgentCompletionDetails(
  agent: IChatAgentData,
  otherAgents: IChatAgentData[],
  chatAgentNameService: IChatAgentNameService,
): { label: string; isDupe: boolean } {
  ...
  const isDupe =
    isAllowed &&
    !!otherAgents.find(
      (other) => other.name === agent.name && other.id !== agent.id,
    )
