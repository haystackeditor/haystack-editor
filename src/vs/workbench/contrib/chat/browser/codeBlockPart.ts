...
export class CodeCompareBlockPart extends Disposable {
  ...
  private readonly _lastDiffEditorViewModel = this._store.add(
    new MutableDisposable<IDiffEditorViewModel>(),
  )
  private currentScrollWidth = 0
  ...
  constructor(
    ...
    const editorScopedService = this.diffEditor
      .getModifiedEditor()
      .contextKeyService.createScoped(toolbarElement)
    const editorScopedInstantiationService =
      scopedInstantiationService.createChild(
        new ServiceCollection([IContextKeyService, editorScopedService]),
      )
    this.toolbar1 = this._register(new ActionBar(toolbarElement, {}))
    this.toolbar2 = this._register(
      editorScopedInstantiationService.createInstance(
        MenuWorkbenchToolBar,
        toolbarElement,
        menuId,
    ...
    this._register(
      this.toolbar2.onDidChangeDropdownVisibility((e) => {
        toolbarElement.classList.toggle("force-visibility", e)
      }),
    )
    ...
    this.toolbar1.push(
      toAction({
        ...
        tooltip: localize(
          "chat.edit.tooltip",
          "Open '{0}'",
          this.labelService.getUriLabel(data.edit.uri, { relative: true }),
        ),
        run: () => {
