...
} from "vs/workbench/contrib/chat/browser/chatAgentHover"
...
import {
  IChatProgressMessageRenderData,
  IChatRenderData,
  ...
  implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate>
{
  ...
  constructor(
    ...
    this.renderer = this._register(
      this.instantiationService.createInstance(ChatMarkdownRenderer, undefined),
    )
          ...
          MenuId.ChatMessageTitle,
          {
            ...
            actionViewItemProvider: (
              ...
              if (
                ...
                (action.item.id === "workbench.action.chat.voteDown" ||
                  action.item.id === "workbench.action.chat.voteUp")
              ) {
  ...
  private basicRenderElement(
      ...
      const result =
        data.kind === "treeData"
          ...
          : data.kind === "markdownContent"
            ...
            : data.kind === "progressMessage" &&
                onlyProgressMessagesAfterI(value, index)
              ? this.renderProgressMessage(data, false) // TODO render command
              : data.kind === "progressTask"
                ? this.renderProgressTask(data, false, element, templateData)
                : data.kind === "command"
                  ? this.renderCommandButton(element, data)
                  : data.kind === "textEditGroup"
                    ? this.renderTextEdit(element, data, templateData)
                    : data.kind === "warning"
                      ? this.renderNotification("warning", data.content)
                      : data.kind === "confirmation"
                        ? this.renderConfirmation(element, data, templateData)
                        : undefined
    ...
    if (isResponseVM(element) && element.errorDetails?.message) {
      const renderedError = this.renderNotification(
    ...
    if (element.isCanceled) {
      ...
      renderableResponse.forEach((part, index) => {
        ...
        if (!renderedPart) {
          if (part.kind === "treeData") {
            ...
            partsToRender[index] = {
              ...
              isAtEndOfResponse: onlyProgressMessagesAfterI(
                renderableResponse,
                index,
              ),
        ...
        // Did this part's content change?
          ...
          isMarkdownRenderData(renderedPart)
        ) {
          // TODO
          const wordCountResult = this.getDataForProgressiveRender(
            element,
          ...
          if (
            ...
            renderedPart.renderedWordCount !== wordCountResult?.actualWordCount
          ) {
            this.traceLayout(
              "doNextProgressiveRender",
        ...
        // Is it a progress message that needs to be rerendered?
          ...
          part.kind === "progressMessage" &&
          isProgressMessageRenderData(renderedPart) &&
          (renderedPart.isAtEndOfResponse !==
            onlyProgressMessagesAfterI(renderableResponse, index) ||
          ...
          isProgressTaskRenderData(renderedPart)
        ) {
          const isSettled = part.isSettled?.() ?? true
          if (
            renderedPart.isSettled !== isSettled ||
            part.progress.length !== renderedPart.progressLength ||
            isSettled
          ) {
            partsToRender[index] = {
              task: part,
      ...
      isFullyRendered =
        partsToRender.filter((p) => !("isSettled" in p) || !p.isSettled)
        ...
        partsToRender.forEach((partToRender, index) => {
          ...
          if (isInteractiveProgressTreeData(partToRender)) {
            ...
            if (onlyProgressMessageRenderDatasAfterI(partsToRender, index)) {
              result = this.renderProgressMessage(
                partToRender.progressMessage,
                index === partsToRender.length - 1,
              )
            } else {
              result = null
            }
          } else if (isProgressTaskRenderData(partToRender)) {
            result = this.renderProgressTask(
              partToRender.task,
              !partToRender.isSettled,
              element,
              templateData,
            )
            ...
            result = this.renderConfirmation(
              element,
              partToRender,
              templateData,
            )
          ...
          // Avoid doing progressive rendering for multiple markdown parts simultaneously
            ...
            const originalMarkdown =
              "originalMarkdown" in part ? part.originalMarkdown : undefined
          ...
          if (existingElement) {
            if (result === null) {
              templateData.value.replaceChild(
                $("span.placeholder-for-deleted-thing"),
                existingElement,
              )
            } else {
  ...
  private renderContentReferencesIfNeeded(
    ...
    if (
      ...
      const contentReferencesListResult = this.renderContentReferencesListData(
        null,
      ...
      if (templateData.referencesListContainer.firstChild) {
        templateData.referencesListContainer.replaceChild(
          ...
          templateData.referencesListContainer.firstChild!,
        )
      } else {
  ...
  private renderContentReferencesListData(
    ...
    const collapseButton = listDisposables.add(
  ...
  private renderProgressTask(
    ...
    element: ChatTreeItem,
    templateData: IChatListItemTemplate,
  ): IMarkdownRenderResult | undefined {
    if (!isResponseVM(element)) {
      return
    }

    if (task.progress.length) {
      const refs = this.renderContentReferencesListData(
        task,
        task.progress,
        element,
        templateData,
  ...
  private renderConfirmation(
    ...
    const confirmationWidget = store.add(
      this.instantiationService.createInstance(
    ...
    store.add(
      confirmationWidget.onDidClick(async (e) => {
        if (isResponseVM(element)) {
          ...
          if (
            await this.chatService.sendRequest(element.sessionId, prompt, data)
          ) {
  ...
  private renderMarkdown(
    ...
    if (isResponseVM(element)) {
      ...
      disposables.add(
        toDisposable(() => this.codeBlocksByResponseId.delete(element.id)),
      )
    }

    disposables.add(
      this.markdownDecorationsRenderer.walkTreeAndAnnotateReferenceLinks(
        result.element,
      ),
    )
...
class ContentReferencesListPool extends Disposable {
  ...
  private listFactory(): WorkbenchList<
    IChatContentReference | IChatWarningMessage
  > {
    const resourceLabels = this._register(
      this.instantiationService.createInstance(ResourceLabels, {
        onDidChangeVisibility: this._onDidChangeVisibility,
    ...
    const list = this.instantiationService.createInstance(
      ...
      ],
      {
        ...
        accessibilityProvider: {
          getAriaLabel: (
            element: IChatContentReference | IChatWarningMessage,
          ) => {
          ...
          getWidgetAriaLabel: () =>
            localize("usedReferences", "Used References"),
        },
        dnd: {
          getDragURI: (
            element: IChatContentReference | IChatWarningMessage,
...
class ContentReferencesListDelegate
  implements IListVirtualDelegate<IChatContentReference | IChatWarningMessage>
{
...
class ContentReferencesListRenderer
  implements
    IListRenderer<
      IChatContentReference | IChatWarningMessage,
      IChatContentReferenceListTemplate
    >
{
  ...
  renderElement(
    data: IChatContentReference | IChatWarningMessage,
    index: number,
...
function isProgressMessageRenderData(
  item: IChatRenderData,
): item is IChatProgressMessageRenderData {
  return item && "isAtEndOfResponse" in item
}

function isCommandButtonRenderData(
  item: IChatRenderData,
): item is IChatCommandButton {
  return item && "kind" in item && item.kind === "command"
}

function isTextEditRenderData(
  item: IChatRenderData,
): item is IChatTextEditGroup {
  return item && "kind" in item && item.kind === "textEditGroup"
}

function isConfirmationRenderData(
  item: IChatRenderData,
): item is IChatConfirmation {
  return item && "kind" in item && item.kind === "confirmation"
}

function isMarkdownRenderData(
  item: IChatRenderData,
): item is IChatResponseMarkdownRenderData {
  return item && "renderedWordCount" in item
}

function onlyProgressMessagesAfterI(
  items: ReadonlyArray<IChatProgressRenderableResponseContent>,
  i: number,
): boolean {
  return items.slice(i).every(isProgressMessage)
}

function onlyProgressMessageRenderDatasAfterI(
  items: ReadonlyArray<IChatRenderData>,
  i: number,
): boolean {
