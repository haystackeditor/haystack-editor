...
export class ChatWidget extends Disposable implements IChatWidget {
  ...
  private _onDidDeleteContext = this._register(
    new Emitter<IChatRequestVariableEntry>(),
  )
  ...
  constructor(
    ...
    CONTEXT_IN_QUICK_CHAT.bindTo(contextKeyService).set(
      "resource" in viewContext,
    )
    ...
    this._register(
      codeEditorService.registerCodeEditorOpenHandler(
        async (
          ...
          if (
            matchesScheme(resource, Schemas.vscodeCopilotBackingChatCodeBlock)
          ) {
  ...
  private createInput(
    ...
    options?: { renderFollowups: boolean; renderStyle?: "default" | "compact" },
  ): void {
    this.inputPart = this._register(
      this.instantiationService.createInstance(ChatInputPart, this.location, {
    ...
    this._register(
      this.inputPart.onDidDeleteContext((e) =>
        this._onDidDeleteContext.fire(e),
      ),
    )
    this._register(
