  ...
  implements IEditorContribution
{
  ...
  constructor(
    ...
    const modelObs = observableFromEvent(editor.onDidChangeModel, () =>
      editor.getModel(),
    )
...
registerAction2(
  class FilterCoverageToTestInEditor extends Action2 {
    ...
    run(accessor: ServicesAccessor, coverageOrUri?: FileCoverage | URI): void {
      ...
      const options =
        coverage?.perTestData ?? coverage?.isForTest?.parent.perTestData
      if (!options) {
        return
      }
      ...
      quickInputService
        ...
        .then((selected) => {
          ...
          testCoverageService.filterToTest.set(
            selected ? selected.item?.isForTest!.id : previousSelection,
            undefined,
          )
        })
    }
