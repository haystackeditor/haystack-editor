...
} from "vs/editor/browser/widget/diffEditor/commands"
  ...
  implements IAccessibleViewImplentation
{
  ...
  getProvider(accessor: ServicesAccessor) {
    ...
    const switchSides = localize(
      "msg3",
      "Run the command Diff Editor: Switch Side<keybinding:diffEditor.switchSide> to toggle between the original and modified editors.",
    )
    ...
    const content = [
      ...
      localize(
        "msg2",
        "View the next<keybinding:{0}> or previous<keybinding:{1}> diff in diff review mode, which is optimized for screen readers.",
