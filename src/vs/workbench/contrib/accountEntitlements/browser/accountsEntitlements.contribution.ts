  ...
  implements IWorkbenchContribution
{
  ...
  private showChatWelcomeViewContextKey = new RawContextKey<boolean>(
    chatWelcomeViewConfigKey,
    false,
  ).bindTo(this.contextService)
  ...
  private registerListeners() {
    ...
    this._register(
      this.authenticationService.onDidChangeSessions(async (e) => {
        if (
          e.providerId === this.productService.gitHubEntitlement!.providerId &&
        ...
        async (e) => {
          if (e.id === this.productService.gitHubEntitlement!.providerId) {
            await this.enableEntitlements(
              (await this.authenticationService.getSessions(e.id))[0],
            )
          }
        },
      ),
  ...
  private async getEntitlementsInfo(
    ...
    if (
      !(this.productService.gitHubEntitlement!.enablementKey in parsedResult) ||
  ...
  private async enableEntitlements(session: AuthenticationSession | undefined) {
    ...
    const showWelcomeView =
      this.configurationService.inspect<boolean>(chatWelcomeViewConfigKey)
        .value ?? false
    ...
    if (enabled) {
      if (isInternal && showWelcomeView) {
        ...
        this.telemetryService.publicLog2<
          { enabled: boolean },
          EntitlementEnablementClassification
        >(chatWelcomeViewConfigKey, { enabled: true })
      }
        ...
        class extends Action2 {
          ...
          public async run(accessor: ServicesAccessor) {
            ...
            const confirmation = await dialogService.confirm({
              ...
              primaryButton:
                productService.gitHubEntitlement!.confirmationAction,
            })
...
configurationRegistry.registerConfiguration({
  ...
  properties: {
    "workbench.chat.experimental.showWelcomeView": {
      ...
      tags: ["experimental"],
      description: localize(
        "workbench.chat.showWelcomeView",
