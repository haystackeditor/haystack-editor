...
class CESContribution extends Disposable implements IWorkbenchContribution {
  ...
  private async promptUser() {
    ...
    const sendTelemetry = (
      userReaction: "accept" | "remindLater" | "neverShowAgain" | "cancelled",
    ) => {
      /* __GDPR__
			"cesSurvey:popup" : {
				"owner": "digitarald",
      ...
      [
        {
          ...
          run: () => {
            ...
            const usedParams = this.productService.surveys
              ?.filter(
                (surveyData) => surveyData.surveyId && surveyData.languageId,
              )
              ...
              .map(
                (surveyData) =>
                  `${encodeURIComponent(surveyData.languageId)}Lang=1`,
              )
              .join("&")
            if (usedParams) {
  ...
  private async schedulePrompt(): Promise<void> {
    ...
    if (remindLaterDate) {
      const timeToRemind =
        new Date(remindLaterDate).getTime() + REMIND_LATER_DELAY - Date.now()
      if (timeToRemind > 0) {
        waitTimeToShowSurvey = timeToRemind
      }
    } else {
      const timeFromInstall =
        Date.now() - new Date(this.telemetryService.firstSessionDate).getTime()
      const isNewInstall =
