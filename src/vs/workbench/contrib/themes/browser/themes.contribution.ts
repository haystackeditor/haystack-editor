...
class InstalledThemesPicker {
  ...
  public async openQuickPick(
    ...
    if (this.extensionGalleryService.isEnabled()) {
      if (
        this.extensionResourceLoaderService.supportsExtensionGalleryResources &&
  ...
  implements IWorkbenchContribution
{
    ...
    setTimeout(async () => {
      ...
      if (await this._hostService.hadLastFocus()) {
        ...
        if (this._workbenchThemeService.hasUpdatedDefaultThemes()) {
          ...
          if (
            currentTheme === ThemeSettingDefaults.COLOR_THEME_LIGHT_OLD ||
            currentTheme === ThemeSettingDefaults.COLOR_THEME_DARK_OLD
          ) {
  ...
  private async _showYouGotMigratedNotification(): Promise<void> {
    const usingLight =
      this._workbenchThemeService.getColorTheme().type === ColorScheme.LIGHT
    const newThemeSettingsId = usingLight
      ? ThemeSettingDefaults.COLOR_THEME_LIGHT
      : ThemeSettingDefaults.COLOR_THEME_DARK
    const newTheme = (await this._workbenchThemeService.getColorThemes()).find(
