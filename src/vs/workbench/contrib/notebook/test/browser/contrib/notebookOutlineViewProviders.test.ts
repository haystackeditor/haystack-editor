...
} from "vs/workbench/contrib/notebook/browser/contrib/outline/notebookOutline"
...
suite("Notebook Outline View Providers", function () {
  ...
  test("OutlinePane 0: Default Settings (Headers Only ON, Code cells OFF, Symbols ON)", async function () {
    ...
    for (const cell of cells) {
      entryFactory
        .getOutlineEntries(cell, OutlineTarget.OutlinePane, 0)
        .forEach((entry) => outlineModel.addChild(entry))
    }

    // Generate filtered outline (view model)
    const outlinePaneProvider = new NotebookOutlinePaneProvider(
      () => [],
  ...
  test("QuickPick 0: Symbols On + 2 cells WITH symbols", async function () {
    ...
    // Generate filtered outline (view model)
    const quickPickProvider = new NotebookQuickPickProvider(
  ...
  test("Breadcrumbs 0: Code Cells On ", async function () {
    ...
    // Generate filtered outline (view model)
    const breadcrumbsProvider = new NotebookBreadcrumbsProvider(
