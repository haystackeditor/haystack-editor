...
const NLS_FIND_IN_CELL_SELECTION_BTN_LABEL = nls.localize(
  "label.findInCellSelectionButton",
...
export abstract class SimpleFindReplaceWidget extends Widget {
  ...
  constructor(
    ...
    this.searchInSelectionEnabled =
      this._configurationService.getValue<boolean>(NotebookSetting.findScope)
    this.inSelectionToggle.domNode.style.display = this.searchInSelectionEnabled
      ? "inline"
      : "none"
  ...
  private setCellSelectionDecorations() {
    ...
    this.selectionDecorationIds = this._notebookEditor.deltaCellDecorations(
      [],
      decorations,
    )
  }
  ...
  public show(
    ...
    if (
      this.searchInSelectionEnabled &&
      options?.searchInRanges !== undefined
    ) {
