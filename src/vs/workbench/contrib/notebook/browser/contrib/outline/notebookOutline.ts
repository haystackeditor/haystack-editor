  ...
  implements IDataSource<NotebookCellOutline, OutlineEntry>
{
  ...
  *getChildren(
    ...
    const showCodeCellSymbols = this._configurationService.getValue<boolean>(
  ...
  constructor(
    ...
    const installSelectionListener = () => {
      ...
      if (!notebookEditor?.hasModel()) {
        ...
        this._outlineProviderReference?.dispose()
        this._localDisposables.clear()
        this._outlineProviderReference = instantiationService.invokeFunction(
          (accessor) =>
            accessor
              .get(INotebookCellOutlineProviderFactory)
              .getOrCreate(notebookEditor, _target),
        )
        this._localDisposables.add(
          this._outlineProviderReference.object.onDidChange((e) => {
    ...
    const options: IWorkbenchDataTreeOptions<OutlineEntry, FuzzyScore> = {
      collapseByDefault:
        ...
        (_target === OutlineTarget.OutlinePane &&
          _configurationService.getValue(OutlineConfigKeys.collapseItems) ===
    ...
    this.config = {
      treeDataSource: instantiationService.createInstance(
      ...
      breadcrumbsDataSource: instantiationService.createInstance(
  ...
  implements IOutlineCreator<NotebookEditor, OutlineEntry>
{
  ...
  async createOutline(
    ...
    const showAllOutlineSymbols = this._configurationService.getValue<boolean>(
      NotebookSetting.outlineShowCodeCellSymbols,
    )
