...
export class InlineChatController implements IEditorContribution {
  ...
  private readonly _ui: Lazy<{
    content: InlineChatContentWidget
  ...
  private async [State.INIT_UI](
    ...
    this._sessionStore.add(
      this._ui.value.content.onDidBlur(() => this.cancelSession()),
    )
    ...
    this._sessionStore.add(
      this._session.chatModel.onDidChange(async (e) => {
        if (e.kind === "addRequest" && e.request.response) {
          ...
          // later request
          const exchange = this._session!.exchanges.find(
            (candidate) => candidate.prompt.request.id === e.requestId,
          )
      ...
      this._languageFeatureService.completionProvider.register(
        { scheme: ChatInputPart.INPUT_SCHEME, hasAccessToAllModels: true },
        {
          ...
          provideCompletionItems: (model, position, context, token) => {
            ...
            if (
              widget !== this._ui.value.zone.widget.chatWidget &&
              widget !== this._ui.value.content.chatWidget
            ) {
              return undefined
            }
    ...
    const updateSlashDecorations = (
      ...
      for (const command of (this._session?.session.slashCommands ?? []).sort(
    ...
    this._sessionStore.add(
      inputInputEditor.onDidChangeModelContent(() =>
        updateSlashDecorations(inputDecorations, inputInputEditor.getModel()!),
      ),
    )
    this._sessionStore.add(
      zoneInputEditor.onDidChangeModelContent(() =>
        updateSlashDecorations(zoneDecorations, zoneInputEditor.getModel()!),
      ),
    )
    this._sessionStore.add(
      toDisposable(() => {
  ...
  > {
    ...
    const request: IChatRequestModel | undefined = chatModel
      .getRequests()
      .at(-1)
    ...
    let next:
      ...
      | State.PAUSE
      | State.ACCEPT
      | State.WAIT_FOR_INPUT = State.SHOW_RESPONSE
    store.add(
    ...
    store.add(
      this._ui.value.zone.widget.chatWidget.inputEditor.onDidChangeModelContent(
        () => {
          this._chatService.cancelCurrentRequestForSession(chatModel.sessionId)
        },
      ),
    ...
    const textModel0Sha1 = sha1.canComputeSHA1(this._session.textModel0)
      ...
      : generateUuid()
    const editState: IChatTextEditGroupState = {
      sha1: textModel0Sha1,
      applied: 0,
    }
        ...
        if (newEdits.length > 0) {
          ...
          progressiveEditsQueue.queue(async () => {
            ...
            if (
              ...
              !this._ui.value.zone.position?.equals(startNow)
            ) {
  ...
  private async [State.SHOW_RESPONSE](): Promise<State.WAIT_FOR_INPUT> {
    ...
    if (response instanceof EmptyResponse) {
      ...
      if (!response.isCancellation) {
        this._ui.value.zone.widget.updateStatus(response.message, {
          classes: ["error"],
        })
        this._strategy?.cancel()
      }
  ...
  private _showWidget(initialRender: boolean = false, position?: Position) {
    ...
    if (this._session && this._ui.rawValue?.zone) {
      this._ui.rawValue?.zone.updateBackgroundColor(
        widgetPosition,
  ...
  updateInput(text: string, selectAll = true): void {
    ...
    if (selectAll) {
      ...
      this._ui.value.zone.widget.chatWidget.inputEditor.setSelection(
        newSelection,
      )
    }
  }
