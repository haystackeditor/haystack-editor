...
export class InlineChatZoneWidget extends ZoneWidget {
        ...
        statusMenuId: {
          ...
          options: {
            buttonConfigProvider: (action) => {
              if (
                new Set([ACTION_REGENERATE_RESPONSE, ACTION_TOGGLE_DIFF]).has(
                  action.id,
                )
              ) {
  ...
  private _availableSpaceGivenIndentation(
    ...
    return (
      info.contentWidth -
      (info.glyphMarginWidth + info.decorationsWidth + (indentationWidth ?? 0))
    )
  }
  ...
  override show(position: Position): void {
    ...
    const marginWithoutIndentation =
      info.glyphMarginWidth + info.decorationsWidth + info.lineNumbersWidth
    ...
    if (position.lineNumber > 1) {
      ...
      const spaceBelowLine =
        this.editor.getScrollHeight() -
        this.editor.getBottomForLineNumber(position.lineNumber)
  ...
  private _calculateIndentationWidth(position: Position): number {
    ...
    return Math.max(
      0,
      this.editor.getOffsetForColumn(indentationLineNumber, indentationLevel),
