...
export class TerminalService extends Disposable implements ITerminalService {
  ...
  setEditable(instance: ITerminalInstance, data?: IEditableData | null): void {
    ...
    const pane =
      this._viewsService.getActiveViewWithId<TerminalViewPane>(TERMINAL_VIEW_ID)
  ...
  async createTerminal(
    ...
    const location =
      (await this.resolveLocation(options?.location)) || this.defaultLocation
  ...
  private _addToReconnected(instance: ITerminalInstance): void {
    ...
    if (reconnectedTerminals) {
      ...
      this._reconnectedTerminals.set(instance.reconnectionProperties.ownerId, [
        instance,
      ])
    }
  }
