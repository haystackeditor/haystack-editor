...
export const terminalSendSequenceCommand = async (
  ...
  if (instance) {
    const text =
      isObject(args) && "text" in args ? toOptionalString(args.text) : undefined
    if (!text) {
      return
    }
...
export function registerTerminalActions() {
  ...
  registerTerminalAction({
    ...
    run: async (c, _, args) => {
      const cwd =
        isObject(args) && "cwd" in args ? toOptionalString(args.cwd) : undefined
...
async function resolveWorkspaceFolderCwd(
  ...
  return isAbsolute(resolvedCwdConfig) ||
    ...
    ? {
        ...
        isAbsolute: true,
        isOverridden: true,
        cwd: URI.from({ scheme: folder.uri.scheme, path: resolvedCwdConfig }),
      }
    : {
        folder,
        isAbsolute: false,
