    ...
    >
{
  ...
  renderElement(
    ...
    templateData.source.textContent = element.sourceData
      ? `${basename(element.sourceData.source.name)}:${element.sourceData.lineNumber}`
      : ""
    templateData.toDispose.push(
      this.hoverService.setupUpdatableHover(
        getDefaultHoverDelegate("mouse"),
        templateData.source,
        element.sourceData
          ? `${this.labelService.getUriLabel(element.sourceData.source.uri)}:${element.sourceData.lineNumber}`
          : "",
      ),
    )
  ...
  protected renderExpression(
    ...
    if (isReplVariable || !expression.name) {
      ...
      renderExpressionValue(
        ...
        data.value,
        { colorize: true, linkDetector: this.linkDetector },
        this.hoverService,
      )
