  ...
  implements ITerminalContribution
{
  ...
  constructor(
    ...
    this._terminalAgentRegisteredContextKey =
      TerminalChatContextKeys.agentRegistered.bindTo(this._contextKeyService)
    ...
    this._responseSupportsIssueReportingContextKey =
      TerminalChatContextKeys.responseSupportsIssueReporting.bindTo(
        this._contextKeyService,
      )
    this._sessionResponseVoteContextKey =
      TerminalChatContextKeys.sessionResponseVote.bindTo(
        this._contextKeyService,
      )
    ...
    this._register(
      this._chatService.onDidPerformUserAction((e) => {
        ...
        if (
          ...
          e.sessionId ===
            this._chatWidget?.rawValue?.inlineChatWidget.getChatModel()
              .sessionId
        ) {
          if (e.action.kind === "bug") {
            ...
            this.acceptFeedback(
              e.action.direction === ChatAgentVoteDirection.Up,
            )
          }
        }
      }),
  ...
  async acceptInput(): Promise<IChatResponseModel | undefined> {
    ...
    try {
      ...
      const supportIssueReporting =
        this._currentRequest?.response?.agent?.metadata?.supportIssueReporting
