  ...
  implements ITerminalAddon, ISuggestController
{
  ...
  private _sync(promptInputState: IPromptInputModelState): void {
    ...
    const enabled =
      config.enabled ||
      this._terminalConfigurationService.config.shellIntegration?.suggestEnabled
    if (!enabled) {
      return
    }
    ...
    if (this._terminalSuggestWidgetVisibleContextKey.get()) {
      const inputBeforeCursor = this._currentPromptInputState.value.substring(
        0,
        this._currentPromptInputState.cursorIndex,
      )
  ...
  private _handleCompletionsSequence(
    ...
    let completionList: IPwshCompletion[] | IPwshCompletion =
      args.length === 0 || payload.length === 0 ? [] : JSON.parse(payload)
    ...
    if (replacementIndex !== 100 && completionList.length > 0) {
      ...
      if (!this._cachedFirstWord) {
        ...
        this._cachedFirstWord.sort((a, b) => {
          ...
          const isANonAlpha =
            aCode < 65 || (aCode > 90 && aCode < 97) || aCode > 122 ? 1 : 0
          const isBNonAlpha =
            bCode < 65 || (bCode > 90 && bCode < 97) || bCode > 122 ? 1 : 0
