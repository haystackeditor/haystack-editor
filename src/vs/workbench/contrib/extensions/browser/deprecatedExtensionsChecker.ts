  ...
  implements IWorkbenchContribution
{
        ...
        for (const { local } of e) {
          if (
            ...
            extensionsWorkbenchService.local.find((extension) =>
              areSameExtensions(extension.identifier, local.identifier),
            )?.deprecationInfo
          ) {
        ...
        [
          {
            ...
            run: async () => {
              this.setNotifiedDeprecatedExtensions(
                toNotify.map((e) => e.identifier.id.toLowerCase()),
              )
  ...
  private setNotifiedDeprecatedExtensions(notified: string[]): void {
    this.storageService.store(
      "extensionsAssistant/deprecated",
      JSON.stringify(
        distinct([...this.getNotifiedDeprecatedExtensions(), ...notified]),
      ),
      StorageScope.PROFILE,
      StorageTarget.USER,
    )
