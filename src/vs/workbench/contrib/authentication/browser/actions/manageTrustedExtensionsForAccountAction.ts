...
export class ManageTrustedExtensionsForAccountAction extends Action2 {
  ...
  override async run(
    ...
    const quickPick = disposableStore.add(
      quickInputService.createQuickPick<TrustedExtensionsQuickPickItem>(),
    )
    quickPick.canSelectMany = true
    quickPick.customButton = true
    quickPick.customLabel = localize("manageTrustedExtensions.cancel", "Cancel")
    const usages = authenticationUsageService.readAccountUsages(
      providerId,
      accountLabel,
    )
    ...
    const toQuickPickItem = function (
      extension: AllowedExtension,
    ): IQuickPickItem & { extension: AllowedExtension } {
