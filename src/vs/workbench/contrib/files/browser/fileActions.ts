...
async function deleteFiles(
  ...
  if (
    skipConfirm ||
    (useTrash &&
      configurationService.getValue<boolean>(CONFIRM_DELETE_SETTING_KEY) ===
        false)
  ) {
  ...
  const onSuccess = async (value: string): Promise<void> => {
    try {
      ...
      if (isFolder) {
        ...
        await haystackService.createFileEditor(resourceToCreate)
      }
    } catch (error) {
      ...
      toPaste.files.length > 1
        ? getFileNamesMessage(
            toPaste.files.map((item) =>
              toPaste.type === "paths" ? item.path : (item as File).name,
            ),
          )
        : undefined
    ...
    if (targets.length) {
      ...
      if (targets.length === 1) {
        ...
        if (item && !item.isDirectory) {
          await haystackService.createFileEditor(item.resource, undefined, {
            preserveFocus: true,
          })
        }
      }
...
export const openFilePreserveFocusHandler = async (
  ...
  for (const stat of stats) {
    await haystackService.createFileEditor(stat.resource, undefined, {
      preserveFocus: true,
    })
  }
}
