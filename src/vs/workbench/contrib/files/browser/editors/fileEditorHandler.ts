...
export class FileEditorInputSerializer implements IEditorSerializer {
  ...
  serialize(editorInput: EditorInput): string {
    ...
    const serializedFileEditorInput: ISerializedFileEditorInput = {
      ...
      editRange:
        fileEditorInput.editRange &&
        `${fileEditorInput.editRange.startLineNumber}-${fileEditorInput.editRange.startColumn}-${fileEditorInput.editRange.endLineNumber}-${fileEditorInput.editRange.endColumn}`,
      isGhost: fileEditorInput.isGhost(),
      xPosition: fileEditorInput.getXPosition() ?? undefined,
      yPosition: fileEditorInput.getYPosition() ?? undefined,
      width: fileEditorInput.getWidth() ?? undefined,
      height: fileEditorInput.getHeight() ?? undefined,
    }
  ...
  deserialize(
    ...
    return instantiationService.invokeFunction((accessor) => {
      ...
      const editRange = serializedFileEditorInput.editRange
      const isGhost = serializedFileEditorInput.isGhost
      const xPosition = serializedFileEditorInput.xPosition
      const yPosition = serializedFileEditorInput.yPosition
      const width = serializedFileEditorInput.width
      const height = serializedFileEditorInput.height

      let deserializedEditRange: IRange | undefined = undefined
      if (editRange) {
        const editRangeStrings = editRange.split("-")
        deserializedEditRange = {
          startLineNumber: Number(editRangeStrings[0]),
          startColumn: Number(editRangeStrings[1]),
          endLineNumber: Number(editRangeStrings[2]),
          endColumn: Number(editRangeStrings[3]),
        }
      }
      ...
      const fileEditorInput = accessor
        ...
        .createTextEditor({
          ...
          languageId,
          forceFile: true,
          editRange: deserializedEditRange,
          isGhost,
          xPosition,
          yPosition,
          width,
