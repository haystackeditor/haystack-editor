...
export class SCMWorkingSetController implements IWorkbenchContribution {
  ...
  constructor(
    ...
    const onDidChangeConfiguration = Event.filter(
      ...
      (e) => e.affectsConfiguration("scm.workingSets.enabled"),
      this._disposables,
    )
    this._disposables.add(
      Event.runAndSubscribe(onDidChangeConfiguration, () =>
  ...
  private _onDidChangeConfiguration(): void {
    if (
      !this.configurationService.getValue<boolean>("scm.workingSets.enabled")
    ) {
            ...
            Event.runAndSubscribe(
              ...
              async () => {
                ...
                const currentHistoryItemGroupId =
                  repository.provider.historyProvider.currentHistoryItemGroup.id
