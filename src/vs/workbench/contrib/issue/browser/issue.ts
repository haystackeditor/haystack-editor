...
} from "vs/workbench/contrib/issue/browser/issueReporterModel"
...
export class BaseIssueReporterService extends Disposable {
  ...
  constructor(
    ...
    this.issueReporterModel = new IssueReporterModel({
      ...
      versionInfo: {
        vscodeVersion: `${product.nameShort} ${
          !!product.darwinUniversalAssetId
            ? `${product.version} (Universal)`
            : product.version
        } (${product.commit || "Commit unknown"}, ${
          product.date || "Date unknown"
        })`,
        os: `${this.os.type} ${this.os.arch} ${this.os.release}${
          isLinuxSnap ? " snap" : ""
        }`,
      },
    ...
    const fileOnProduct = data.issueSource === IssueSource.Haystack
