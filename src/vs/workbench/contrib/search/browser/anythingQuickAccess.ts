...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
  ...
  private get configuration() {
    const editorConfig =
      this.configurationService.getValue<IWorkbenchEditorConfiguration>()
        .workbench?.editor
    const searchConfig =
      this.configurationService.getValue<IWorkbenchSearchConfiguration>().search
  ...
  private async getFilePicks(
    ...
    return fileMatches
      .filter(
        (resource) =>
          !excludes.has(resource) && !resource.path.startsWith("/scratchpad/"),
      )
        ...
        tooltip:
          openSideBySideDirection === "right"
            ? localize(
                {
                  key: "openToSide",
                  comment: ["Open this file in a new editor"],
                },
                "Force open in new editor",
              )
            : localize(
                {
                  key: "openToBottom",
  ...
  private async openAnything(
    ...
    if (URI.isUri(resourceOrEditor)) {
      this.haystackService.createFileEditor(resourceOrEditor, {
        forceNewEditor: options.keyMods?.shift || options.forceOpenSideBySide,
      })
    } else if (isEditorInput(resourceOrEditor)) {
      if (resourceOrEditor.resource != null) {
        this.haystackService.createFileEditor(resourceOrEditor.resource, {
          forceNewEditor: options.keyMods?.shift || options.forceOpenSideBySide,
        })
      }
    } else {
