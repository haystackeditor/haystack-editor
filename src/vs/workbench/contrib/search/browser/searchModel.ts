    ...
    if (model && !isAiContributed) {
      ...
      if (this.rawMatch.results) {
        this.rawMatch.results.filter(resultIsMatch).forEach((rawMatch) => {
          textSearchResultToMatches(rawMatch, this, isAiContributed).forEach(
            (m) => this.add(m),
          )
        })
      }
...
export class FolderMatch extends Disposable {
  ...
  private disposeMatches(): void {
    ...
    ;[...this._unDisposedFileMatches.values()].forEach((fileMatch: FileMatch) =>
      fileMatch.dispose(),
    )
...
export class SearchResult extends Disposable {
  ...
  private _aiFolderMatchesMap: TernarySearchTree<URI, FolderMatchWithResource> =
    TernarySearchTree.forUris<FolderMatchWorkspaceRoot>((key) =>
      this.uriIdentityService.extUri.ignorePathCasing(key),
    )
    ...
    try {
      ...
      await Promise.all(
        elementsToReplace.map(async (elem) => {
          ...
          if (
            (parent instanceof FolderMatch || parent instanceof FileMatch) &&
      ...
      elementsToRemove.forEach((currentElement) => {
        if (!arrayContainsElementOrParent(currentElement, removedElems)) {
          currentElement
            .parent()
            .remove(
              <(FolderMatch | FileMatch)[] & Match & FileMatch[]>currentElement,
            )
