...
export abstract class BaseWindow extends Disposable implements IBaseWindow {
  ...
  protected setWin(win: BrowserWindow): void {
    ...
    if (
      useCustomTitleStyle &&
      ((isWindows && useWindowControlsOverlay(this.configurationService)) ||
        isMacintosh)
    ) {
  ...
  protected applyState(
    state: IWindowState,
    hasMultipleDisplays = screen.getAllDisplays().length > 0,
  ): void {
...
export class CodeWindow extends BaseWindow implements ICodeWindow {
  ...
  private registerListeners(): void {
    ...
    this._register(
      Event.fromNodeEventEmitter<ElectronEvent>(
        this._win.webContents,
        "will-prevent-unload",
    ...
    if (!e || e.affectsConfiguration("http.proxy")) {
      ...
      if (
        ...
        if (typeof (app as appWithProxySupport).setProxy === "function") {
          ;(app as appWithProxySupport).setProxy({
            proxyRules,
            proxyBypassRules,
            pacScript: "",
          })
