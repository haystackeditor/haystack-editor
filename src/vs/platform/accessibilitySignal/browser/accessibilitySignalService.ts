  ...
  implements IAccessibilitySignalService
{
  ...
  public getDelayMs(
    ...
    const delaySettingsKey =
      signal.delaySettingsKey ?? "accessibility.signalOptions.delays.general"
    const delaySettingsValue: { sound: number; announcement: number } =
      this.configurationService.getValue(delaySettingsKey)
    return modality === "sound"
      ? delaySettingsValue.sound
      : delaySettingsValue.announcement
  }
}
