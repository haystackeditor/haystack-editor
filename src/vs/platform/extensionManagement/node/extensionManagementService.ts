...
import {
  fromExtractError,
  getManifest,
} from "vs/platform/extensionManagement/node/extensionManagementUtil"
  ...
  implements INativeServerExtensionManagementService
{
  ...
  private async downloadAndExtractGalleryExtension(
    ...
    const { verificationStatus, location } =
  ...
  implements IInstallExtensionTask
{
  ...
  protected async doRun(token: CancellationToken): Promise<ILocalExtension> {
    ...
    const { local, verificationStatus } = await this.extractExtensionFn(
      this.operation,
      token,
    )
