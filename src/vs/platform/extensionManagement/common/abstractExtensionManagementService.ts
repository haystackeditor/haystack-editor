  ...
  implements IExtensionManagementService
{
            ...
            try {
              ...
              for (const { gallery, manifest } of distinct(
                ...
                if (existingInstallingExtension) {
                  if (
                    this.canWaitForTask(task, existingInstallingExtension.task)
                  ) {
                    const identifier =
                  ...
                  !installed.some(({ identifier }) =>
                    areSameExtensions(identifier, gallery.identifier),
                  )
                ) {
  ...
  private async uninstallExtension(
    ...
    const getUninstallExtensionTaskKey = (identifier: IExtensionIdentifier) =>
      `${identifier.id.toLowerCase()}${uninstallOptions.versionOnly ? `-${extension.manifest.version}` : ""}${uninstallOptions.profileLocation ? `@${uninstallOptions.profileLocation.toString()}` : ""}`
        ...
        for (const packedExtension of packedExtensions) {
          if (
            this.uninstallingExtensions.has(
              getUninstallExtensionTaskKey(packedExtension.identifier),
            )
          ) {
            this.logService.info(
              "Extensions is already requested to uninstall",
