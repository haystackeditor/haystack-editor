...
export class Scrollable extends Disposable {
  private shouldIgnoreScrollEvents = false
  ...
  public forceScrollTopForFiveFrames(top: number): void {
    // Set the scroll position to the specified top value
    this.setScrollPositionNow({ scrollTop: top })

    // Prevent any scroll events from hijacking.
    this.shouldIgnoreScrollEvents = true

    // Schedule to revert back to the original scroll position after five frames
    this.clearScrollTopAfterNFrames(0, 5)
  }
