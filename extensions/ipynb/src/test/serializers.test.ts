...
;[true, false].forEach((useCustomPropertyInMetadata) => {
  suite(
    `ipynb serializer (${useCustomPropertyInMetadata ? "with custom metadata (standard behaviour)" : "without custom metadata"})`,
    () => {
        ...
        sinon
          ...
          .callsFake((section, scope) => {
            if (section === "jupyter") {
              ...
              ).wrappedMethod.call(vscode.workspace, section, scope)
            }
          })
      })
      teardown(async () => {
      ...
      test("Deserialize", async () => {
        ...
        expectedCodeCell.metadata = useCustomPropertyInMetadata
          ? { custom: { execution_count: 10, metadata: {} } }
