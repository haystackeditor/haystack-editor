...
;[true, false].forEach((useCustomPropertyInMetadata) => {
  suite(
    `Notebook Model Store Sync (${useCustomPropertyInMetadata ? "with custom metadata (standard behaviour)" : "without custom metadata"})`,
    () => {
        ...
        sinon
          ...
          .callsFake((section, scope) => {
            if (section === "jupyter") {
              ...
              return (workspace.getConfiguration as any).wrappedMethod.call(
                workspace,
                section,
                scope,
              )
      ...
      test("Add cell id if nbformat is 4.5", async () => {
        if (useCustomPropertyInMetadata) {
          sinon
            ...
            .get(() => ({ custom: { nbformat: 4, nbformat_minor: 5 } }))
        } else {
          sinon
            .stub(notebook, "metadata")
