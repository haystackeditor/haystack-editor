...
export class AzureActiveDirectoryService {
  ...
  private async handleCodeResponse(
    ...
    return new Promise(
      (resolve: (value: vscode.AuthenticationSession) => void, reject) => {
        uriEventListener = this._uriHandler.event(async (uri: vscode.Uri) => {
          try {
            ...
            const verifier =
              ...
              this._codeVerfifiers.get(decodeURIComponent(nonce))
            if (!verifier) {
  ...
  private async fetchTokenResponse(
    ...
    if (
      ...
      endpointUrl =
        proxyEndpoints?.microsoft || this._env.activeDirectoryEndpointUrl
    }
  ...
  private async checkForUpdates(
    ...
    for (const key of e.added) {
      ...
      const matchesExisting = this._tokens.some(
        (token) =>
          token.scope === session.scope && token.sessionId === session.id,
      )
