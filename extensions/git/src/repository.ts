...
export class Repository implements Disposable {
  ...
  async stage(resource: Uri, contents: string): Promise<void> {
    const path = relativePath(this.repository.root, resource.fsPath).replace(
      /\\/g,
      "/",
    )
  ...
  async getBranches(
    ...
    return await this.run(Operation.GetBranches, async () => {
      ...
      return refs.filter(
        (value) =>
          (value.type === RefType.Head || value.type === RefType.RemoteHead) &&
    ...
    return await this.run(Operation.Show, async () => {
      ...
      try {
        ...
        if (err.gitErrorCode === GitErrorCodes.WrongCase) {
          const gitRelativePath = await this.repository.getGitRelativePath(
            ref,
            path,
          )
          return await this.repository.bufferString(
  ...
  async ignore(files: Uri[]): Promise<void> {
    return await this.run(Operation.Ignore, async () => {
      ...
      const textToAppend = files
        .map((uri) =>
          relativePath(this.repository.root, uri.fsPath).replace(/\\/g, "/"),
        )
        .join("\n")
