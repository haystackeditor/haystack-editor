...
function setupGlobals(vscode) {
  ...
  const api = {
    ...
    getConfig: (relativePath) => {
      ...
      return {
        mode:
          config["hotReload.sources"][relativePath] === "patch-prototype"
            ? "patch-prototype"
            : undefined,
      }
    },
  ...
  store.add(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (
        e.affectsConfiguration("vscode-diagnostic-tools.debuggerScriptsConfig")
      ) {
        update()
      }
    }),
  ...
  store.add(
    vscode.commands.registerCommand(
      ...
      async (relativePath) => {
        ...
        const newValue =
          current === "patch-prototype" ? undefined : "patch-prototype"
        config = {
          ...config,
          "hotReload.sources": {
            ...config["hotReload.sources"],
